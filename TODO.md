...
- [x] 名前空間・ディレクトリでモジュール分割（packwerk 導入、5パック構成）→ ADR-007
- [ ] モジュール間インターフェース定義（`app/public/` への切り出しは未実施）
- [ ] イベント基盤の導入
- [ ] 通知機能（イベント駆動で実装）
  - [ ] 注文確定通知
  - [ ] 在庫切れ通知（出品者向け）
- [ ] Product の inverse 関連（cart_items / order_items / coupon）の cascade 設計（package_todo.yml に棚上げ中）
