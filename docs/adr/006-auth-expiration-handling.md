...
### 案A: `apiFetch` で401を検知し、signOut→ログイン画面にリダイレクト

共通API呼び出し関数（`frontend/lib/api.ts`）で `res.status === 401` を検知し、
NextAuthの `signOut({ redirectTo: "/auth/login" })` を呼ぶ。

- メリット: 変更箇所が1ファイルで済む。JWT有効期限を変えても追従する
...
- 全Server ActionのAPI呼び出しで401時に自動サインアウトが走る
